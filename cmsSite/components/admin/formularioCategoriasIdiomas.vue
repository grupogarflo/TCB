        ...
        <v-row>
          ...
          <v-col cols="12">
            <v-text-field
              ...
              required
              label="Orden"
              @input="$v.editedItem.orden.$touch()"
              @blur="$v.editedItem.orden.$touch()"
            />
          </v-col>
...
export default {
  ...
  computed: {
    formTitle() {
      ...
      return this.idRegistroSend === 0 ? 'Nueva categoría' : 'Editar categoría'
    },
    nameErrors() {
      const errors = []
    ...
    orderErrors() {
      const errors = []
      if (!this.$v.editedItem.orden.$dirty) {
        return errors
      }
      !this.$v.editedItem.orden.required && errors.push('Orden es requerido.')
      return errors
    },
    metaTitleErrors() {
      const errors = []
  ...
  methods: {
    ...
    save() {
      ...
      if (
        ...
        this.isLoading = true
        const url =
          this.idRegistroSend === 0 ? '/addCategories' : '/updateCategories'
          ...
          .then((response) => {
            ...
            this.textoAlertaMesaje =
              this.idRegistroSend === 0
                ? 'Se a creado la categoria, no olvides dar de alta todos los idiomas'
                : 'Se a modificado la categoria'
