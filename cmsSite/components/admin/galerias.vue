    ...
    <v-container fluid>
      <v-row>
        ...
        <v-col cols="12">
          <v-card-actions>
            <v-spacer />
            <v-btn v-show="btnSave" color="blue darken-1" text @click="close"
              >Cancelar</v-btn
            >
            <v-btn v-show="btnDelete" color="blue darken-1" text @click="save"
              >Guardar</v-btn
            >
          </v-card-actions>
        </v-col>
...
export default {
  ...
  props: {
    ...
    galeriaGet: { required: true },
    urlAddGaleria: { required: true },
  },
  data: () => ({
    isLoading: false,
    alertMensajes: false,
  ...
  computed: {
    mapaErrors() {
      ...
      !this.$v.editedItem.mapa.required &&
        errors.push('el código de la galeria es requerido.')
      return errors
    },
  },
  ...
  methods: {
          ...
          .then((response) => {
            ...
            this.textoAlertaMesaje =
              this.idRegistroSend === 0
                ? 'Se a agregado la galeria al tour'
                : 'Se a modificado la galeria'
    ...
    setconf() {
      this.editedItem.mapa = this.galeriaGet
      // console.log(this.galeriaGet)
      if (this.galeriaGet !== '') this.existeMapa = true
    },
