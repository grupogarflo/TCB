                ...
                <v-col cols="12">
                  <v-card>
                    <v-tabs
                      ...
                      :key="componentKey"
                    >
                      <!--<v-tab @click="clickTap('1')">Español</v-tab>
                      <v-tab @click="clickTap('3')">Ingles</v-tab>-->
                      <v-tab
      ...
      <v-dialog v-model="dialogoEliminacion" max-width="450">
        <v-card>
          <v-card-title class="headline"
            >Eliminación de categorías</v-card-title
          >
...
import formularioCategoriasIdiomas from '~/components/admin/formularioCategoriasIdiomas'

export default {
  components: { formularioCategoriasIdiomas },
  ...
  computed: {
    formTitle() {
      ...
      return this.idRegistro === 0 ? 'Nueva categoría' : 'Editar categoría'
    },
  },
  ...
  methods: {
    async getRegistros() {
      try {
        await this.$axios.get('/getAllCategories', {}).then((resp) => {
    ...
    deleteItem(item) {
      ...
      this.idRegistro = item.id
      this.textoDialogoEliminacion = `¿Desea realizar la eliminación de la categoria: ${item.name}?`
    },
      ...
      this.$axios
        ...
        .catch((error) => {
          ...
          this.textoDialogoEliminacion =
            'La categoría que esta intentado eliminar no existe o ya fue eliminado previamente'
        })
    },
