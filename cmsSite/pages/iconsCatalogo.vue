  ...
  >
    <template v-slot:top>
      <v-toolbar flat color="white">
        <v-toolbar-title>Catálogo de icons</v-toolbar-title>
      ...
      <v-dialog v-model="dialogoEliminacion" max-width="450">
        <v-card>
          <v-card-title class="headline">Eliminación del icon</v-card-title>
...
<script>
import FormularioIcons from '~/components/admin/FormularioIcons'

export default {
  components: { FormularioIcons },
  ...
  computed: {
    formTitle() {
      ...
      return this.idRegistro === 0 ? 'Nuevo icon' : 'Editar icon'
    },
  },
  ...
  methods: {
    async getRegistros() {
      try {
        await this.$axios.get('/getAllIcon', {}).then((resp) => {
    ...
    deleteItem(item) {
      ...
      this.idRegistro = item.id
      this.textoDialogoEliminacion = `¿Desea realizar la eliminación del  icon: ${item.name_esp}?`
    },
      ...
      this.$axios
        ...
        .catch((error) => {
          ...
          this.textoDialogoEliminacion =
            'El icon  que esta intentado eliminar no existe o ya fue eliminado previamente'
        })
    },
